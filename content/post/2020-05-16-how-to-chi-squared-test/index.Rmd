...
To get the expected values $E_k$, we can multiply the probabilities by the number of total observations (in this case, `r sum(observed)`):
...
Or, if we built a distribution function out of our samples, our value would fall into the `r chisq_percentile` percentile. Since that is definitely larger than 0.95, we can conclude that there is an association between the number of gears and the number of cylinders. Alternatively, this is what the Chi-squared test would have returned:
...
> A chi-square test was conducted whether there is an association between the number of gears and the number of cylinders across different cars. The results were significant (χ^2^(`r rs$parameter`) = `r round(rs$statistic,2)`, p = `r round(rs$p.value,3)`)
